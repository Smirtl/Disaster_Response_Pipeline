...
def load_data(database_filepath):
    '''
    Takes in the SQL database filename from the process script and returns features (messages),
    target variable (categories) and the category columns.
    '''
...
def tokenize(text):
    '''
    NLP script to tokenize text. Takes in the text string and returns tokens.
    '''
...
def build_model():
    '''
    Builds a machine learning pipeline using grid search. Returns the
    Grid Search object.
    '''
...
def evaluate_model(model, X_test, Y_test, category_names):
    '''
    Evaluates the model after running it. Takes in the model, test data and category names.
...
def save_model(model, model_filepath):
    '''
    Saves the model as a pickle file. Takes in the model and a chosen filepath.
    '''
